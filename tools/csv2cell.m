function [METRICS,Info] = csv2cell(varargin)
        ...
        if thisChar == delimiter
            if inField 
                if inQuoteField % this comma is part of the field
                    ...
                    METRICS{lineNumber,fieldNumber} = field;
                    field = '';
            ...
            if charNumber == nChars % this is a hanging comma, indicating the last field is blank
                METRICS{lineNumber,fieldNumber} = '';
                field = '';
                fieldNumber = fieldNumber + 1;
            end
        elseif thisChar == '"' 
            ...
            if thisChar == decimal
                thisChar = '.'; % Sustituimos por el decimal de Matlab
            end
            field(end+1) = thisChar;
...
% Creamos el archivo Info a partir de la cabecera del CSV
Info = struct;
[row_end, col_end] = size(METRICS);
for col=1:col_end
    valor_cabecera = METRICS{1,col};
    Info.(valor_cabecera) = col;
end

% Eliminamos la cabecera del cell principal
METRICS = METRICS(2:row_end,:);
row_end = row_end - 1;

% Repasamos todos los campos del cell para convertir a numérico si corresponde
for row=1:row_end
    for col=1:col_end
        valor = str2double(METRICS{row,col});
        if ~isnan(valor)
            METRICS{row,col} = str2double(METRICS{row,col});
        end
