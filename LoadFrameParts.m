function [frameParts]=LoadFrameParts(yuvSequence, frame, mode, qp, csfMode, bestmodeby)
%frameParts=LoadFrameParts(frameFileName, frame, mode, qp, csfmode, bestmodeby);
%   Detailed explanation goes here

[pathstr,sequenceName,~] = fileparts(yuvSequence);

if strcmp('hevc',mode)
    pattern=sprintf('%s\\%s\\%s\\%s_%s_%03d_frameParts_*qp%02d_%s_bm%s.mat',pathstr,sequenceName,mode,mode,sequenceName,frame,qp,csfMode,bestmodeby);
else
